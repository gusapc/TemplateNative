...
import styles from './Apendix1ScreenStyle';
import sentences4 from './apendix1';
import originVerbs from './originVerbsApendix1';
import verbs from './dataApendix1';
...
export default function Apendix1Screen({navigation}) {
	const scrollViewRef = useRef();

	const appendixTwoLoader = useApiSimpleObj('APPENDIX_TWO');
	const [list, setList] = useState([]);
	const [fullVerbs, setFullVerbs] = useState(false);
	const [isRandom, setIsRandom] = useState(false);

	let listRandom = appendixTwoLoader.data.listRandom;
	let randomIndex = appendixTwoLoader.data.randomIndex;
	let current = appendixTwoLoader.data.current || 0;
	...
	const setCurrent = (n) => {
		appendixTwoLoader.setData({ current: n });
	};
	...
	const shuffleArray = (array) => {
		...
		appendixTwoLoader.setData({ randomIndex: indices });
		return newArray;
	};
	...
	useLayoutEffect(() => {
		...
		if (!appendixTwoLoader?.data?.listRandom) {
			appendixTwoLoader.setData({ listRandom: shuffleArray(sentences4) });
		}
		if (!appendixTwoLoader?.data?.current) {
			appendixTwoLoader.setData({ current: 0 });
		}
		setList(sentences4);
	}, []);
		...
		<View style={[styles.container, { backgroundColor: 'black' }]}>
			<SafeAreaView style={[styles.container, { backgroundColor: 'black' }, styles.justifyContentSpaceBetween]}>
				...
				>
					...
					/>
					<Entypo
						onPress={() => appendixTwoLoader.setData({ listRandom: shuffleArray(sentences4) })}
						name={'shuffle'}
						size={30}
						color="white"
					/>
