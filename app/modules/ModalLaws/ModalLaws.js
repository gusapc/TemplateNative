...
import styles from './ModalLawsStyle';

const ModalLaws = (props) => (
	<ModalTitle {...props}>
		<React.Fragment>
			{[
				...
				`Estas condiciones rigen a “Vios”, los productos, las funciones, los servicios las tecnologías y el Software que ofrecemos, excepto cuando indiquemosexpresamente que se aplican otras condiciones distintas a estas.`,
				`1. DEFINICIONES`,
				...
				`El servicio de gestión estará a cargo de nuestro Administrador de Zona quien podrá brindar soporte asesoría en la gestión del fraccionamiento, facilitando información, contactos y procesos que permitan dar solución a contingencias, emergencias, o problemáticas que puedan poner en riesgo la calidad de vida.`,
				`Dicha gestión está encaminada a mejorar siempre la calidad de servicios que se reciben, aumentar la recaudación de cuotas de administración, incentivar la participación, difundir prácticas exitosas en otras colonias o fraccionamientos, dar a conocer casos de éxito.`,
				`3.-PRIVACIDAD`,
				...
				`ViOS no es responsable del contenido que se genere , no tiene ninguna influencia en las publicaciones dentro de la plataforma por parte de los usuarios, Representante vecinal y/o Comité de administración ViOS no se hace responsable por alguna falla o anomalía en los servicios brindados por los prestadores de servicios pues ViOS solo es el medio de contacto no el prestador de servicios.`,
			].map((item, index) => (
...
export default React.memo(ModalLaws);
